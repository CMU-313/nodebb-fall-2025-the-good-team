	...
	Compose.init = function () {
		...
		if (container.length) {
			...
			if (window.currentUser && window.currentUser.role === 'student') {
				const imageUploadSection = container
					.find('.composer-footer .formatting-bar, .composer .formatting-bar')
					.first();
				if (imageUploadSection.length) {
					const dropdownContainer = $(
						'<div id="post-visibility-dropdown" style="margin-top:10px;"></div>',
					);
