...
module.exports = function (Categories) {
	Categories.getCategoryTopics = async function (data) {
		...
		try {
			...
			const rows$decor = await posts.getPostsFields(mainPids$decor, [
				'visibility',
			]); // aligned with mainPids
			const pidToVis$decor = {};
			mainPids$decor.forEach((pid, i) => {
				pidToVis$decor[pid] = rows$decor[i] && rows$decor[i].visibility;
			});

			topicsData.forEach((t) => {
				const vis = t.mainPid
					? pidToVis$decor[t.mainPid] || 'everyone'
					: 'everyone';
				t.isInstructorOnly = vis === 'all_instructors';
				t.isPrivateToInstructor =
					typeof vis === 'string' && vis.startsWith('user:');
			});
		} catch (e) {}

		const isInstructor = await groups.isMember(data.uid, 'instructors');
		const mainPids = topicsData.map((topic) => topic.mainPid);
		const postData = await posts.getPostsFields(mainPids, [
			'visibility',
			'uid',
		]);
		const tidToPostData = {};
		topicsData.forEach((t, i) => {
			tidToPostData[t.tid] = postData[i];
		});
