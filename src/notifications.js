...
Notifications.create = async function (data) {
	...
	try {
		if (data.bodyLong) {
			let text = utils.stripHTMLTags(String(data.bodyLong || ''), [
				'img',
				'p',
				'a',
			]);
		...
		winston.verbose(
			'[notifications.create] failed to generate snippet for bodyShort',
			e,
		);
	}
...
Notifications.merge = async function (notifications) {
	...
	function isGenericShort(str) {
		...
		return /\[\[.*\]\]|user-posted-to|has posted a reply to|mentioned you|mentioned you in|user-posted-topic/i.test(
			str,
		);
	}
				...
				case 'notifications:activitypub.announce':
					{
						...
						if (numUsers === 2 || numUsers === 3) {
							const candidate = `[[notifications:user-posted-to-${typeFromLength(usernames)}, ${usernames.join(', ')}${titleEscaped}]]`;
