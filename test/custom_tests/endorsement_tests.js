...
describe('Endorsement Feature Unit Tests', function () {
	...
	describe('Endorsement.getEndorsementLevel', function () {
		it('should return "none" for zero or negative upvotes', function () {
			assert.strictEqual(
				Endorsement.getEndorsementLevel(0),
				'none',
				'0 upvotes should be "none"',
			);
			assert.strictEqual(
				Endorsement.getEndorsementLevel(-5),
				'none',
				'Negative upvotes should be "none"',
			);
		});

		it('should return "good" for 1 or 2 upvotes (Level 1)', function () {
			assert.strictEqual(
				Endorsement.getEndorsementLevel(1),
				'good',
				'1 upvote should be "good"',
			);
			assert.strictEqual(
				Endorsement.getEndorsementLevel(2),
				'good',
				'2 upvotes should be "good"',
			);
		});

		it('should return "very-helpful" for 3, 4, or 5 upvotes (Level 2)', function () {
			assert.strictEqual(
				Endorsement.getEndorsementLevel(3),
				'very-helpful',
				'3 upvotes should be "very-helpful"',
			);
			assert.strictEqual(
				Endorsement.getEndorsementLevel(5),
				'very-helpful',
				'5 upvotes should be "very-helpful"',
			);
		});

		it('should return "high" for 6 or more upvotes (Level 3)', function () {
			assert.strictEqual(
				Endorsement.getEndorsementLevel(6),
				'high',
				'6 upvotes should be "high"',
			);
			assert.strictEqual(
				Endorsement.getEndorsementLevel(100),
				'high',
				'100 upvotes should be "high"',
			);
		});
	});
	...
	describe('Endorsement.updatePostEndorsement Logic', function () {
		...
		it('should attempt to remove the badge if upvotes is 0, even if hasEndorsement is true (fallback logic)', function () {
			...
			assert.strictEqual(
				result.action,
				'remove',
				'Badge should be removed if upvotes are 0',
			);
		});
	});
});
