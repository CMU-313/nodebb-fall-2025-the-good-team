...
describe('UI Test: Visibility Button Integration (socket + UI)', function () {
    ...
    `);
		...
		alertsAlertMock = function (arg) {
			alertsAlertMock.called = true;
			alertsAlertMock.lastCall = arg;
		};
		alertsAlertMock.called = false;
		alertsAlertMock.lastCall = null;

		alertsErrorMock = function (arg) {
			alertsErrorMock.called = true;
			alertsErrorMock.lastCall = arg;
		};
		alertsErrorMock.called = false;
		alertsErrorMock.lastCall = null;

		// Set up global socket and alerts that reference the mocks
