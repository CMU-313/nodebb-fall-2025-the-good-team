...
define('endorsement', ['hooks'], function (hooks) {
	...
	Endorsement.checkPostEndorsement = function (pid) {
		...
		if (upvotes === 0) {
			const $voteElements = $post.find(
				'[component="post/vote-count"], .vote-count, [data-votes]',
			);
			$voteElements.each(function () {
				const $el = $(this);
				const votes =
					parseInt($el.text()) || parseInt($el.attr('data-votes')) || 0;
				if (votes > 0) {
					upvotes = votes;
					return false; // break
				}
	...
	Endorsement.showEndorsementBadge = function (pid, upvotes) {
		...
		if ($post.length && upvotes > 0) {
			...
			let $target = $post
				.find('[component="post/header"] [itemprop="author"]')
				.parent();
			if (!$target.length) {
